...
header-includes:
- "`\\usepackage{fontspec}`{=latex}"
- "`\\usepackage{polyglossia}`{=latex}"
- "`\\setmainlanguage[babelshorthands=true]{german}`{=latex}"
- "`\\usepackage{hyperref}`{=latex}"
- "`\\usepackage{color}`{=latex}"
- "`\\usepackage{xcolor}`{=latex}"
- "`\\usepackage[misc]{ifsym}`{=latex}"
- "`\\definecolor{darkblue}{rgb}{0,0,.5}`{=latex}"
- "`\\definecolor{darkgreen}{rgb}{0,.5,0}`{=latex}"
- "`\\definecolor{islamicgreen}{rgb}{0.0, 0.56, 0.0}`{=latex}"
- "`\\definecolor{darkred}{rgb}{0.5,0,0}`{=latex}"
- "`\\definecolor{mintedbg}{rgb}{0.95,0.95,0.95}`{=latex}"
- "`\\definecolor{arsenic}{rgb}{0.23, 0.27, 0.29}`{=latex}"
- "`\\definecolor{prussianblue}{rgb}{0.0, 0.19, 0.33}`{=latex}"
- "`\\definecolor{coolblack}{rgb}{0.0, 0.18, 0.39}`{=latex}"
- "`\\hypersetup{colorlinks=true, breaklinks=true, anchorcolor=blue,linkcolor=white, citecolor=islamicgreen, filecolor=darkred,  urlcolor=darkblue}`{=latex}"
- "`\\usepackage{booktabs}`{=latex}"
- "`\\usepackage{pgf}`{=latex}"
- "`\\usepackage{minted}`{=latex}"
- "`\\RequirePackage{fancyvrb}`{=latex}"
- "`\\DefineVerbatimEnvironment{verbatim}{Verbatim}{fontsize=\\scriptsize}`{=latex}"
...
-   Folien und Code auf
    [github.com/kirchnergo/course.2025.hwr.fun](https://github.com/kirchnergo/course.2025.hwr.fun)

## Was ist F#

F# ist eine funktionale Programmiersprache, die das Schreiben von
korrektem und verwaltbarem Code erleichtert. In F# können Typen und
Funktionen geschrieben werden, deren Typen automatisch generalisiert
werden. Dies ermöglicht es, sich auf die Problemdomäne zu konzentrieren
und die Daten zu bearbeiten, statt sich um die Details der
Programmierung zu kümmern.
...
-   [try.fsharp.org](https://try.fsharp.org/) oder
    [fable.io*repl*](https://fable.io/repl/)
...
``` {#code-version .shell exports="both"}
code --version
```
...
-   [Erste Schritte in F# mit der .NET
    Core-CLI](https://docs.microsoft.com/de-de/dotnet/fsharp/get-started/get-started-command-line)
...
-   Initialisiere ein git-Repo (z.B. `course.2025.hwr.fun`{.verbatim})
    -   (pull von github)
...
1.  
    ...
    `\null`{=latex}`\hfill`{=latex}-- Alan Perlis
...
``` {.fsharp results="value" exports="both"}
let s = "hello"  // string
let i = 42       // int
let f = 3.141    // float
let b = true     // bool
...
``` {.fsharp results="output silent" exports="code"}
type Suit = 
    | Hearts 
    | Clubs 
    | Diamonds 
...
![](../img/CollectionFunctions.png "fig:collection-functions"){#fig:collection-functions}
...
1.  
    ...
    `\null`{=latex}`\hfill`{=latex}-- Pablo Picasso
...
1.  
    ...
    `\null`{=latex}`\hfill`{=latex}-- Martin Fowler

# Funktionen \[4/4\]

## Basics

``` {.fsharp exports="both" results="value"}
let squareIt1 n = n * n
...
-   [wikipedia.org/../F Sharp (programming
    language)](https://en.wikipedia.org/wiki/F_Sharp_(programming_language))
-   [wikipedia.org/../List of programming languages by type#Functional
    languages](https://en.wikipedia.org/wiki/List_of_programming_languages_by_type#Functional_languages)
...
-   [ ] [Tour durch
    F#](https://docs.microsoft.com/de-de/dotnet/fsharp/tour)
